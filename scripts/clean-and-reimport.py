...
AIRTABLE_TOKEN = "patwUzfjD9FXYflII.3ff6962deea260b59152184cdc4e6ff448b2903e74d459d0530d6d631db6b9d9"
BASE_ID = "appyd4Tt9kwX4VzWv"
TABLE_ID = "tbl4jcKxkQb4REasY"
...
def parse_date(raw):
    ...
    if re.match(r'^\d{4}$', s):
        return None
    mdy = re.match(r'^(\d{1,2})/(\d{1,2})/(\d{2,4})$', s)
    if mdy:
...
def parse_notes(raw):
    ...
    if re.match(r'^\d{4}$', s):
        return s
    if re.match(r'^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|Spring|Summer|Fall|Winter)', s, re.I):
        return s
...
def get_all_record_ids():
    ...
    offset = None
    while True:
        url = f"{TABLE_URL}?fields[]=Name"
        if offset:
            url += f"&offset={offset}"
        data, _ = airtable_request("GET", url)
...
def delete_records(ids):
    for i in range(0, len(ids), 10):
        batch = ids[i:i+10]
        params = "&".join(f"records[]={id}" for id in batch)
        url = f"{TABLE_URL}?{params}"
        airtable_request("DELETE", url)
        print(f"Deleted {min(i+10, len(ids))} / {len(ids)}")
        time.sleep(0.25)
...
def import_records(rows):
    ...
    for i in range(0, len(rows), 10):
        ...
        records = []
        for row in batch:
            date_raw, name, city, state, type_, description, source = row
            iso_date = parse_date(date_raw)
            notes = parse_notes(date_raw)
            fields = {
                "Church name": name.strip(),
                "City": city.strip(),
                "State": state.strip(),
                "Incident type": map_type(type_),
                "Status": "Done",
            }
            if description and description.strip():
                fields["Description"] = description.strip()
            if source and source.strip():
                fields["Sources/Citations"] = source.strip()
            if iso_date:
                fields["Date"] = iso_date
            if notes:
                fields["Notes"] = notes
            records.append({"fields": fields})
        airtable_request("POST", TABLE_URL, {"records": records})
        imported += len(batch)
        print(f"Imported {imported} / {len(rows)}")
        time.sleep(0.25)

print("Step 1: Fetching existing record IDs...")
ids = get_all_record_ids()
print(f"Found {len(ids)} existing records.")

if ids:
    print("Step 2: Deleting all existing records...")
    delete_records(ids)
    print("Done deleting.")

print(f"Step 3: Importing {len(CSV_ROWS)} records...")
import_records(CSV_ROWS)
print(f"Complete! {len(CSV_ROWS)} records imported cleanly.")
